...
all: unit-tests test image-test lint
...
.PHONY: $(ARTIFACTS)/test-linux-amd64
$(ARTIFACTS)/test-linux-amd64:
	@$(MAKE) local-test-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: test-linux-amd64
test-linux-amd64: $(ARTIFACTS)/test-linux-amd64  ## Builds executable for test-linux-amd64.

.PHONY: test
test: test-linux-amd64  ## Builds executables for test.
...
.PHONY: image-test
image-test:  ## Builds image for test.
	@$(MAKE) target-$@ TARGET_ARGS="--tag=$(REGISTRY)/$(USERNAME)/test:$(TAG)"
